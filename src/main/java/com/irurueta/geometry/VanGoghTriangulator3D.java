...
public class VanGoghTriangulator3D extends Triangulator3D{
    ...
    public List<Triangle3D> triangulate(Polygon3D polygon) 
        ...
        List<Point3D> verticesCopy = new ArrayList<Point3D>(
                polygon.getVertices());
    ...
    public List<Triangle3D> triangulate(List<Point3D> vertices) 
            throws TriangulatorException{
        if(vertices.size() < MIN_VERTICES) throw new TriangulatorException();

        //triangulation will modify provided list of vertices so we make a copy 
        //of it        
        List<Point3D> verticesCopy = new ArrayList<Point3D>(vertices);
        return internalTriangulate(verticesCopy, null, vertices);
    }
    ...
    private static List<Triangle3D> internalTriangulate(
        ...
        List<Triangle3D> result = new LinkedList<Triangle3D>();
                ...
                if (i == 0){                    
                    ...
                    if(triangle == null){
                        ...
                        triangle = new Triangle3D(
                                verticesCopy.get(lastElement - 1),
                    ...
                    if(triangle == null){
                        //instantiate triangle if not already instantiated
                        triangle = new Triangle3D(verticesCopy.get(i - 1),
        ...
        if(triangle == null){
           ...
           triangle.setVertices(verticesCopy.get(0),
                   verticesCopy.get(1), verticesCopy.get(2));
        }
    ...
    public static boolean isEar(Triangle3D triangle, 
        ...
        for(Point3D testPoint : polygonVertices){
            ...
            //polygon is the ear            
            if(triangleVertices.contains(testPoint)) continue;
            ...
            //it is not an ear
            if(isInside || isReversed) return false;
        }
