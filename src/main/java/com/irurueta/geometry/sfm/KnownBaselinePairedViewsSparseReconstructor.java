...
/**
 * Class in charge of estimating pairs of cameras and 3D reconstructed points from sparse
 * image point correspondences from multiple views and known initial camera baseline
 * (camera separation), so that cameras and reconstructed points are obtained with exact
 * scale.
 */
public class KnownBaselinePairedViewsSparseReconstructor extends
        BasePairedViewsSparseReconstructor<
                ...
                KnownBaselinePairedViewsSparseReconstructorListener> {
     ...
     */
    public KnownBaselinePairedViewsSparseReconstructor(
            KnownBaselinePairedViewsSparseReconstructorConfiguration configuration,
            KnownBaselinePairedViewsSparseReconstructorListener listener)
    ...
    public KnownBaselinePairedViewsSparseReconstructor(
            KnownBaselinePairedViewsSparseReconstructorListener listener)
            throws NullPointerException {
        this(new KnownBaselinePairedViewsSparseReconstructorConfiguration(), listener);
    }
    ...
    protected boolean postProcessOne(boolean isInitialPairOfViews) {
        try {
            ...
            for (int i = 0; i < numPoints; i++) {
                metricReconstructedPoints3D.add(mMetricReconstructedPoints.get(i).
                        getPoint());
            }
