...
import com.irurueta.geometry.calib3d.estimators.RadialDistortionRobustEstimator;
import com.irurueta.geometry.calib3d.estimators.RadialDistortionRobustEstimatorListener;
...
public class AlternatingCameraCalibrator extends CameraCalibrator{
    ...
    public void calibrate() throws CalibrationException, LockedException, 
        ...
        try{
            ...
            do{
                ...
                if(!mEstimateRadialDistortion) break;
            ...
            if(bestIntrinsic != null) mIntrinsic = bestIntrinsic;
            if(bestDistortion != null) mDistortion = bestDistortion;
    ...
    protected double estimateRadialDistortion(
        ...
        for(CameraCalibratorSample sample : mSamples){
            ...
            //keep undistorted points for this sample, as they will be used
            //on next iteration to refine homography estimation
//            sample.setUndistortedMarkers(transformedIdealPatternMarkers);                        
        }                
        ...
        try{
            ...
            for(CameraCalibratorSample sample : mSamples){
                if(sample.getHomography() == null) continue;
            ...
            //compare undistortedPoints (obtained by projecting ideal points
            //with estimated camera) with undistortedPoints2 (obtained after
            //undoing distortion with estimated distortion)
            /*Point2D undistortedPoint1, undistortedPoint2;
            totalPoints = undistortedPoints.size();
            for(int i = 0; i < totalPoints; i++){
                undistortedPoint1 = undistortedPoints.get(i);
                undistortedPoint2 = undistortedPoints2.get(i);

                avgError += undistortedPoint1.distanceTo(
                        undistortedPoint2);
                
            }*/
            
            avgError /= (double)inlierCount; // (double)totalPoints;
