...
class Accuracy3DTest {
    ...
    void testConstructor() throws AlgebraException, GeometryException {
        for (var t = 0; t < TIMES; t++) {
            ...
            final var covarianceMatrix = rotationMatrix.multiplyAndReturnNew(
            ...
            assertEquals(accuracy.getStandardDeviationFactor() * (semiAxesLengths[0] + semiAxesLengths[1]
                            + semiAxesLengths[2]) / 3.0, accuracy.getAverageAccuracy(), ABSOLUTE_ERROR);
            assertEquals(3, accuracy.getNumberOfDimensions());
            ...
            assertArrayEquals(ellipsoid2.getSemiAxesLengths(), ArrayUtils.multiplyByScalarAndReturnNew(semiAxesLengths,
            ...
            assertThrows(IllegalArgumentException.class, () -> new Accuracy3D(wrong1));
    ...
    void testGetSetCovarianceMatrix() throws AlgebraException {
        ...
        assertThrows(IllegalArgumentException.class, () -> accuracy.setCovarianceMatrix(wrong2));
    ...
    void testFlattenTo2D() throws AlgebraException, GeometryException {
        ...
        assertEquals(flattenedEllipse.getSemiMajorAxis(), ellipse.getSemiMajorAxis(), ABSOLUTE_ERROR);
        assertEquals(flattenedEllipse.getSemiMinorAxis(), ellipse.getSemiMinorAxis(), ABSOLUTE_ERROR);
        // because ellipses are symmetric, there is a rotation ambiguity
        assertTrue(Math.abs(ellipse.getRotationAngle() - flattenedEllipse.getRotationAngle())
                <= ABSOLUTE_ERROR || Math.abs(Math.abs(ellipse.getRotationAngle()
    ...
    void testSerializeDeserialize() throws WrongSizeException, NonSymmetricPositiveDefiniteMatrixException, IOException,
        ...
        final var accuracy1 = new Accuracy3D(covarianceMatrix, conf);

        // check
        assertSame(covarianceMatrix, accuracy1.getCovarianceMatrix());
        assertTrue(accuracy1.getStandardDeviationFactor() > 0.0);
        assertEquals(conf, accuracy1.getConfidence(), 0.0);

        // serialize and deserialize
        final var bytes = SerializationHelper.serialize(accuracy1);
        final var accuracy2 = SerializationHelper.<Accuracy3D>deserialize(bytes);
