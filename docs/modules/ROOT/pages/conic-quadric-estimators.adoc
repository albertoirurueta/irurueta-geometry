...
constrained: a circle is the conic latexmath:[ax^2+2bxy+cy^2+2dxw+2eyw+fw^2=0] with latexmath:[a=c],
latexmath:[b=0] (`ConicType.CIRCLE_CONIC_TYPE`, see xref:conics-quadrics.adoc[]); a sphere is the quadric with
latexmath:[a=b=c] and latexmath:[d=e=f=0].
...
removes 2 of those degrees of freedom *and* removes the scale ambiguity at the same time: once latexmath:[a=c]
is forced non-zero, the whole equation can be divided by it, leaving the non-homogeneous circle equation
latexmath:[x^2+y^2+2dx+2ey+f=0] with exactly 3 free real numbers latexmath:[(d,e,f)] (equivalently: center
latexmath:[(-d,-e)] and radius latexmath:[\sqrt{d^2+e^2-f}]). That is why
...
by the 5 constraints latexmath:[a=b=c], latexmath:[d=e=f=0] to the 4 free reals latexmath:[(g,h,i,j)] in
latexmath:[x^2+y^2+z^2+2gx+2hy+2iz+j=0] -- is why `SphereRobustEstimator.MINIMUM_SIZE = 4`
...
latexmath:[(2x_iw_i,\ 2y_iw_i,\ w_i^2)] of `A` and the entry latexmath:[b_i=-x_i^2-y_i^2] (each row normalized
...
[latexmath]
++++
d(\text{point}) = \bigl|\, \lVert \text{point} - \text{center} \rVert - \text{radius} \,\bigr|
++++
...
A conic latexmath:[ax^2+2bxy+cy^2+2dxw+2eyw+fw^2=0] has 6 homogeneous coefficients, hence 5 degrees of freedom
...
30-31): each point places one linear constraint latexmath:[(x_i^2,x_iy_i,y_i^2,x_i,y_i,1)\cdot(a,b,c,d,e,f)^{\mathsf T}=0]
...
describes -- for each point the row latexmath:[(x^2,\ 2xy,\ y^2,\ 2xw,\ 2yw,\ w^2)] (row-normalized for
...
[latexmath]
++++
r(m) = \bigl|\, m^{\mathsf T} C\, m \,\bigr|, \qquad r(l) = \bigl|\, l^{\mathsf T} C^{*}\, l \,\bigr|
++++

computed by normalizing latexmath:[C]/latexmath:[C^{*}] and the point/line first, then literally multiplying
...
latexmath:[m^{\mathsf T}Cm] is a genuinely different (and much smaller, for normalized inputs near the locus)
...
A quadric latexmath:[ax^2+by^2+cz^2+2dxy+2exz+2fyz+2gxw+2hyw+2izw+jw^2=0] has 10 homogeneous coefficients,
...
latexmath:[(x^2,\ y^2,\ z^2,\ 2xy,\ 2xz,\ 2yz,\ 2xw,\ 2yw,\ 2zw,\ w^2)] of a 9&#215;10 matrix (row-normalized),
...
[latexmath]
++++
r(M) = \bigl|\, M^{\mathsf T} Q\, M \,\bigr|, \qquad r(\Pi) = \bigl|\, \Pi^{\mathsf T} Q^{*}\, \Pi \,\bigr|
++++
...
* xref:conics-quadrics.adoc[] -- the conic/dual-conic/quadric/dual-quadric math itself: the matrix
  representation, tangent lines/planes, duality (latexmath:[C^{*}=C^{-1}]) and transformation rules that the
